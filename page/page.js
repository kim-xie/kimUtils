;(function($){
	...
	$.extend($.tzPageRenderers.defaultRenderer.prototype, {
		...
		getLinks:function(current_page, eventHandler,psize) {
			...
			//显示可跳转分页
			if(this.opts.showGo){
				fragment.append("<a href='javascript:void(0);' style='float:left;'>共<label class='tmui_page_itemcount'>"+this.maxentries+"</label>条</a>&nbsp;<a href='javascript:void(0);' title='前往当前页' style='' class='tm_go'>跳转</a><input type='text' title='请输入其他页码' class='tm_number' value='"+proxySzie+"' id='tm_pagego'/>");
			}
	...
	$.fn.kimPage = function(maxentries, opts){
		...
		opts = jQuery.extend({
			...
			select_per_page:[5,10,20,50,100,200],//每页显示的个数
